...
/** Wednesdays and Saturdays can only be won by sending a disc into the past. */
...
export function dailyPuzzle(iso: string = todayIso()): Puzzle | null {
  ...
  const puzzle: Puzzle | null = found && {
    ...
    hint: found.byTravel
      ? 'A disc sent into the past can finish it — pull one of yours out and drop it there.'
      : 'It is a drop or a spin on a board that is waiting — check every board, not just this one.',
    state: found.state,
    player: found.player,
    within: 1,
